...
- [x] 实现基础多轮上下文：短回复继承上一轮需求，不重复追问。
- [x] 实现房源排序：位置、预算、户型、空置、面积、更新时间。
- [x] 实现推荐结果结构化输出：房源卡片、推荐原因、风险提示。
- [x] 实现客服话术生成。
- [x] 实现前端聊天界面。
- [x] 实现推荐房源卡片。
- [x] 实现一键复制客服话术。
- [x] 梳理现有意图和规则总表：[RULES.md](RULES.md)。
- [x] 实现客服反馈：合适、不合适、已复制、客户要看房。
- [x] 记录完整调用日志：原始需求、结构化需求、MCP 参数、推荐结果、话术、反馈。
- [x] 实现事件埋点：发送需求、位置解析、MCP 调用、推荐展示、话术复制、反馈提交。
- [x] 确保 MCP Token 和模型 API Key 只保存在服务端。
...
- [x] 实现地铁沿线查询：按已配置线路站点逐个查库存。
- [x] 实现具体地铁站查询：只查目标站点并按距离排序。
...
- 当前已支持 `3号线沿线` 和 `具体地铁站` 两类查询；地铁前后几站仍需要后续地铁拓扑数据。
